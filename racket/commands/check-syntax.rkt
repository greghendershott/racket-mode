...
;; returning a `(break)` response, and, how racket-repl.el handles
;; that by doing nothing (except cleaning up the nonce->callback
;; hash-table).
(define check-syntax
...
(define (do-check-syntax path-str code-str)
  ...
  (parameterize ([current-load-relative-directory (path-only path)])
    ...
    (with-handlers ([exn:fail? handle-fail])
      (define xs (remove-duplicates (drracket/show-content stx)))
