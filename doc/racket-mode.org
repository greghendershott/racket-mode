...
* Architecture
Racket Mode consists of a single Emacs front end, and one or more back ends written in Racket.[fn:pkg]

A back end is responsible for commands that cannot be implemented in Emacs Lisp, as well as supplying zero or more REPLs.

Although you can start and stop a back end with ~racket-start-back-end~ and ~racket-stop-back-end~, a back end is normally started automatically when the front end needs to issue some command.

This includes commands that do /not/ involve ~racket-run~ or a REPL. For example ~racket-xp-mode~ issues commands to check your code and annotate the buffer, even if you do not run a file.

In other words, a back end hosts zero or more REPLs --- it is not the same thing as a REPL.

In the common case there is only one back end on the local host, and it supplies zero or more REPLs.
...
As one example, you can have multiple back ends on the local host. One back end is used for a project under a specific subdirectory, and the other back end for all else. (Perhaps one project needs Racket built from source, and everything else uses an installed, older version of Racket. Not only will files run using either specific version of Racket, so will commands for documentation or visiting definitions.)

#+BEGIN_EXAMPLE
        Emacs front end

          Racket back end for /path/to/project
            REPL
            REPL
            ...
...
Furthermore, you could work with a project located on a remote host, whose files you edit using TRAMP. You also want the back end to run there, so your files run there.
...
#+BEGIN_EXAMPLE
        Emacs front end
          ...
          Racket back end for /ssh:user@host:/
            REPL
            REPL
            ...
#+END_EXAMPLE

For a remote host, the Emacs front end will copy the Racket Mode back end source files to the remote when necessary, and run the back end using ssh.
