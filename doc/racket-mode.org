...
- ~racket-mode~: A major mode to edit ~.rkt~ files. Can be enhanced by enabling any/all of the following minor modes:

  - {{{ref(racket-xp-mode)}}}: Explain and explore code, similar to background check-syntax in Dr Racket.

  - {{{ref(racket-hash-lang-mode)}}}: Use behavior specified by a ~#lang~ for colors, indent, expression navigation, etc. /Experimental/.
...
#+BEGIN_QUOTE
Note: The optional minor mode {{{ref(racket-hash-lang-mode)}}} disables all of the following behavior and uses colors determined by the #lang.
#+END_QUOTE
...
#+BEGIN_QUOTE
Note: The optional minor mode {{{ref(racket-hash-lang-mode)}}} disables all of the following behavior and uses indentation determined by the #lang.
#+END_QUOTE
