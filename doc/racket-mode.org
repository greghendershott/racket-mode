...
Racket Mode supports the Emacs commands ~xref-find-definitions~ {{{kbd(M-.)}}}, ~xref-find-references~ {{{kbd(M-?)}}}, and ~xref-pop-marker-stack~ {{{kbd(M-\,)}}} by supplying an "xref back end" for each of several modes:

- {{{ref(racket-mode)}}}
- {{{ref(racket-xp-mode)}}}
- {{{ref(racket-repl-mode)}}}

Racket doesn't have the concept of a global or project-wide database of definitions and references. Furthermore, various modules can export identifiers with the same symbolic value, for example a different ~define~ is provided by ~racket/base~, ~typed/racket/base~, and other modules. In addition, a module can import, rename, contract, and re-export things.

As a result, to find a definition, it is necessary to know exactly /which/ identifier --- either by expanding the module (as is done by ~racket-xp-mode~) or by actually running it (~racket-repl-mode~). Once known, we can usually find the definition site, even through a chain of renaming and/or contract-wrapping exports. In addition, when point is in an absolute or relative  ~require~ form, we can usually find the source file. (In plain ~racket-mode~ edit buffers not enhanced by ~racket-xp-mode~, the only thing that ~xref-find-definitions~ does is visit relative requires, e.g. ~foo.rkt~ in ~(require "foo.rkt")~.)

As for finding references, the default xref implementation is used, which greps for strings among a project's files. Although ~racket-xp-mode~ can sometimes do better, using ~drracket/check-syntax~ for /intra/ file definitions and references, beyond those it also falls back to the default implementation.
...
Tip: If you use ~paredit~, by default it steals {{{kbd(M-?)}}} for ~paredit-convolute-sexp~. You can either change that binding in ~paredit-mode-map~ or pick some other key for ~xref-find-references~.
