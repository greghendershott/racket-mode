...
You use ~auto-mode-alist~ to tell Emacs which major mode to use initially for certain file extensions. Also, in a buffer you can use ~M-x racket-mode~ and ~M-x racket-hash-lang-mode~ to switch between them.

Note that ~racket-hash-lang-mode~ is derived from ~racket-mode~, so it runs both ~racket-mode-hook~ and ~racket-hash-lang-mode-hook~.
