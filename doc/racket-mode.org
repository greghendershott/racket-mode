...
In Emacs use `M-x list-packages` to list your installed packages.
Find `racket-mode` and press `U` to mark it to be upgraded.
Then press `x` to execute the upgrades.
