...
- ~racket-repl-mode~: A major mode for a Racket REPL. This uses a "back end server" written in Racket, which is responsible for running files and implementing commands that cannot be implemented in Emacs Lisp.[fn:pkg]

- Modes to support specific features, such as:
  - {{{ref(racket-logger-mode)}}}
  - {{{ref(racket-profile-mode)}}}
  - {{{ref(racket-check-syntax-mode)}}}
...
Of course that means there must exist some Racket namespace. When you first visit a ~.rkt~ file, you need to {{{kbd(C-c C-c)}}} to {{{ref(racket-run)}}}. That makes the namespace match the file, and makes its identifiers available to Racket Mode. (Otherwise, as a fallback, Racket Mode uses the same list of identifiers for which it has defined {{{ref(Font-lock (syntax highlighting))}}}.)

Similarly, after you change the file, including its ~require~ forms, you'll need to {{{kbd(C-c C-c)}}} again to make the resulting set of identifiers available for completion.

Optional: You can enable a minor mode to get completion candidates from local defintions: {{{see(racket-check-syntax-mode)}}}.
...
A more satisfying experience is to use {{{ref(racket-describe)}}} or {{{ref(racket-doc)}}}.
